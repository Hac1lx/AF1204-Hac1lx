...
def _(pd):
    ...
    _fyear_candidates = [c for c in sp_df.columns if c.lower() in ("fyear", "fiscal_year", "year", "fiscalyear")]
    ...
    if "Market_Cap" not in sp_df.columns and _mcap_candidates:
        sp_df = sp_df.rename(columns={_mcap_candidates[0]: "Market_Cap"})
    elif "Market_Cap" not in sp_df.columns:
        sp_df["Market_Cap"] = float("nan")

    # Detect company-name column
    _name_candidates = [c for c in sp_df.columns if c.lower() in ("name", "company", "ticker", "conm", "gvkey")]
    if "Name" not in sp_df.columns and _name_candidates:
        sp_df = sp_df.rename(columns={_name_candidates[0]: "Name"})
    elif "Name" not in sp_df.columns:
        sp_df["Name"] = sp_df.index.astype(str)

    _required = ["AvgCost_of_Debt", "Z_Score_lag", "Sector_Key", "fyear"]
    sp_df = sp_df.dropna(subset=[c for c in _required if c in sp_df.columns])
...
def _():
    try:
        ...
        raise ImportError("plotly is not installed. Run: pip install plotly") from _e
    return (px,)
...
def _(metric_labels, metric_select, mo, n_firms, px, year_range, yearly_avg):
    ...
    fig_trend = px.line(
        ...
        title=(
            f"{_lbl} by Sector "
            f"({year_range.value[0]} to {year_range.value[1]}) "
...
def _(mo, sp_df):
    ...
    treemap_metric = mo.ui.dropdown(
        options=_treemap_options,
        value="Total Market Cap (B)",
        label="Size Metric",
    )
    treemap_labels = {v: k for k, v in _treemap_options.items()}
...
def _(mo, px, sp_df, treemap_labels, treemap_metric, treemap_year):
    ...
    fig_tree = px.treemap(
        ...
        title=f"S&P 500 Market Composition by Sector and Company — {_yr}",
        template="presentation",
        width=920,
        height=600,
    )
...
def _(mo, pd, px, sp_df):
    ...
    fig_esg = px.bar(
        esg_df.sort_values("Count"),
        x="Count", y="Keyword", color="Category",
        orientation="h",
    ...
    fig_hist = px.histogram(
        ...
        color="Sector_Key",
        nbins=50,
        title="Distribution of Altman Z-Scores Across S&P 500 Companies (all years)",
    ...
    return chart_esg, chart_hist, esg_df, fig_esg, fig_hist
...
def _(mo):
    ...
    shots_in = mo.ui.dropdown(
        options=_shot_options,
        value="Zero-shot (no examples)",
        label="Prompting Strategy",
    )
    shot_labels = {v: k for k, v in _shot_options.items()}
    return context_in, persona_in, shot_labels, shots_in, task_in, temp_in


@app.cell
def _(context_in, mo, persona_in, shot_labels, shots_in, task_in, temp_in):
    _t   = temp_in.value
    ...
    if shots_in.value == 1:
        ...
        _ex = "\n## FEW_SHOT_EXAMPLES\nEx1: Z=1.2 -> High risk. Ex2: Z=2.5 -> Grey zone. Ex3: Z=3.8 -> Low risk, healthy.\n"
    ...
    _prompt = (
        ...
        "2. Identify financial risk signals relevant to the acquisition.\n"
        ...
        "## OUTPUT_FORMAT\n"
        "Max 6 bullet points. End with Go / No-Go recommendation."
    )
    llm_out = mo.vstack([
        mo.callout(mo.md(
            f"**Temperature = {_t}** ({_lbl}) | **Strategy:** {_shot_lbl}"
        ), kind="info"),
...
| AI and LLMs | Prompt engineering, RAG, grounding, AI-as-judge |
| Dev Tools | Marimo, GitHub Codespaces, GitHub Pages WASM |
| Office | Excel formulas and data handling, Word, PowerPoint |
...
@app.cell
def _(mo, tab_about, chart_trend, metric_labels, metric_select, trend_sectors, year_range, n_firms,
       chart_tree, treemap_year, treemap_metric, treemap_labels,
       chart_esg, chart_hist,
       llm_out, persona_in, task_in, context_in, temp_in, shots_in, shot_labels,
       chart_map, yearly_avg):
    tab_analysis = mo.vstack([
        mo.md("## Z-Score and Cost of Debt Trend Analysis"),
        mo.callout(mo.md(
            "Time series analysis of sector-level financial health across the S&P 500. "
            "Switch metric using the dropdown. Adjust year range with the range slider."
        ), kind="info"),
...
- Reactive re-aggregation — Week 4 reactivity
        """),
    ])
    tab_comp = mo.vstack([
        mo.md("## S&P 500 Market Composition Treemap"),
        mo.callout(mo.md(
            "Hierarchical treemap broken down by sector then individual company. "
            "Colour scale runs from red (low value) to green (high value)."
        ), kind="info"),
...
- Reactive year and metric selection — Week 4
    ...
    tab_pipeline = mo.vstack([
        ...
        mo.md("### Part A — ESG Keyword Frequency and Z-Score Distribution (Week 7)"),
        mo.callout(mo.md(
            ...
            "or pytesseract OCR. Histogram shows Z-Score spread across all companies."
        ), kind="warn"),
        chart_esg,
...
- Script 3: PDF download, PyMuPDF/pytesseract, df_DL.csv ledger
        ...
        mo.callout(mo.md(
            "Structured prompt builder with zero-shot, one-shot, and few-shot strategies. "
            "Grounding context field simulates Retrieval-Augmented Generation (RAG)."
        ), kind="info"),
        ...
        mo.md("""
**Skills demonstrated:**
- Full prompt template structure — Week 8
- Few-shot prompting strategy switching — Week 8
- Temperature and Top_P awareness — Week 8
- RAG simulation via grounding context — Week 8
...
- Reactive prompt builder — self-exploration
    ...
    tab_personal = mo.vstack([
        ...
        mo.md(
            "Growing up in Faisalabad and moving to London for university gave me "
            "a cross-cultural perspective that shapes how I approach business and "
            "data problems. I am particularly interested in international finance "

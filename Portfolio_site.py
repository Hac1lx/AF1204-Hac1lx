# /// script
# requires-python = ">=3.13"
# dependencies = [
#     "marimo>=0.19.10",
#     "pandas>=2.3.3",
#     "plotly>=6.5.1",
# ]
# ///

import marimo

__generated_with = "0.19.11"
app = marimo.App()


# Cell 1: Page Header 
@app.cell
def _(mo):
    mo.md(r"""
    ---
    ## AF1204 Individual Portfolio
    Hassan Ahmad Chaudhary · BSc Accounting and Finance · Bayes Business School
    """)
    return


# Cell 2: Core Imports 
@app.cell
def _():
    import marimo as mo
    import pandas as pd

    # micropip is available in the WASM/Pyodide environment (GitHub Pages).
    # On a local desktop run it won't exist, so we catch the ImportError.
    try:
        import micropip as micropip
    except ImportError:
        micropip = None          # local run — pip-installed packages used directly

    return micropip, mo, pd


# Cell 3: Data Loading 
@app.cell
def _(pd):
    from io import StringIO
    _URL = (
        "https://gist.githubusercontent.com/DrAYim/"
        "80393243abdbb4bfe3b45fef58e8d3c8/raw/"
        "ed5cfd9f210bf80cb59a5f420bf8f2b88a9c2dcd/"
        "sp500_ZScore_AvgCostofDebt.csv"
    )
    try:
        # WASM / GitHub Pages (Pyodide) — use open_url, not pd.read_csv(url)
        from pyodide.http import open_url as _open_url
        sp_df = pd.read_csv(StringIO(_open_url(_URL).read()))
    except ImportError:
        sp_df = pd.read_csv(_URL)           # local desktop run

    sp_df.columns = sp_df.columns.str.strip()

    # Normalise fiscal-year column name
    _fy = [c for c in sp_df.columns if c.lower() in ("fyear","fiscal_year","year","fiscalyear")]
    if "fyear" not in sp_df.columns and _fy:
        sp_df = sp_df.rename(columns={_fy[0]: "fyear"})
    elif "fyear" not in sp_df.columns:
        _dc = [c for c in sp_df.columns if "date" in c.lower() or "yr" in c.lower()]
        sp_df["fyear"] = pd.to_datetime(sp_df[_dc[0]], errors="coerce").dt.year if _dc else 2020

    # Normalise market-cap and name columns
    _mc = [c for c in sp_df.columns if "cap" in c.lower() or "mktcap" in c.lower()]
    if "Market_Cap" not in sp_df.columns:
        sp_df = sp_df.rename(columns={_mc[0]: "Market_Cap"}) if _mc else sp_df.__setitem__("Market_Cap", float("nan")) or sp_df
    _nc = [c for c in sp_df.columns if c.lower() in ("name","company","ticker","conm","gvkey")]
    if "Name" not in sp_df.columns:
        sp_df = sp_df.rename(columns={_nc[0]: "Name"}) if _nc else sp_df.__setitem__("Name", sp_df.index.astype(str)) or sp_df

    _req = ["AvgCost_of_Debt","Z_Score_lag","Sector_Key","fyear"]
    sp_df = sp_df.dropna(subset=[c for c in _req if c in sp_df.columns])
    sp_df = sp_df[sp_df["AvgCost_of_Debt"] < 5]
    sp_df["Cost_Pct"] = sp_df["AvgCost_of_Debt"] * 100
    sp_df["MCap_B"]   = pd.to_numeric(sp_df["Market_Cap"], errors="coerce") / 1e9
    sp_df["fyear"]    = pd.to_numeric(sp_df["fyear"], errors="coerce").astype("Int64")
    sp_df = sp_df.dropna(subset=["fyear"])
    sp_df["fyear"]    = sp_df["fyear"].astype(int)
    return (sp_df,)


# Cell 4: Install & Import Plotly (WASM-safe) 
@app.cell
async def _(micropip):
    # In the WASM environment, plotly must be installed at runtime via micropip.
    # On a local run micropip is None so we skip the install and just import.
    if micropip is not None:
        await micropip.install("plotly")
    import plotly.express as px
    return (px,)


# Cell 5: Trend UI Controls 
@app.cell
def _(mo, sp_df):
    all_sectors = sorted(sp_df["Sector_Key"].unique().tolist())
    all_years   = sorted(sp_df["fyear"].unique().tolist())

    trend_sectors = mo.ui.multiselect(
        options=all_sectors,
        value=all_sectors[:5],
        label="Sectors to Plot",
    )
    year_range = mo.ui.range_slider(
        start=int(min(all_years)),
        stop=int(max(all_years)),
        step=1,
        value=[int(min(all_years)), int(max(all_years))],
        label="Year Range",
    )
    _metric_options = {
        "Average Cost of Debt (%)": "Cost_Pct",
        "Average Altman Z-Score":   "Z_Score_lag",
        "Average Market Cap (B)":   "MCap_B",
    }
    metric_select = mo.ui.dropdown(
        options=_metric_options,
        value="Average Cost of Debt (%)",
        label="Metric",
    )
    metric_labels = {v: k for k, v in _metric_options.items()}
    return all_sectors, all_years, metric_labels, metric_select, trend_sectors, year_range


# Cell 6: Trend Aggregation 
@app.cell
def _(metric_labels, metric_select, sp_df, trend_sectors, year_range):
    _col = metric_select.value
    _lbl = metric_labels.get(_col, _col)
    _df  = sp_df[
        (sp_df["Sector_Key"].isin(trend_sectors.value)) &
        (sp_df["fyear"] >= year_range.value[0]) &
        (sp_df["fyear"] <= year_range.value[1])
    ]
    yearly_avg = (
        _df.groupby(["fyear","Sector_Key"])[_col]
        .mean().round(3).reset_index()
        .rename(columns={_col: _lbl, "fyear": "Year", "Sector_Key": "Sector"})
    )
    n_firms = _df["Name"].nunique()
    return n_firms, yearly_avg


# Cell 7: Trend Chart 
@app.cell
def _(metric_labels, metric_select, mo, n_firms, px, year_range, yearly_avg):
    _lbl = metric_labels.get(metric_select.value, metric_select.value)
    fig_trend = px.line(
        yearly_avg,
        x="Year", y=_lbl, color="Sector", markers=True,
        title=(
            f"{_lbl} by Sector "
            f"({year_range.value[0]}–{year_range.value[1]}) "
            f"| {n_firms} unique companies"
        ),
        template="presentation", width=900, height=520,
    )
    fig_trend.update_traces(line=dict(width=2.5))
    fig_trend.update_layout(legend=dict(orientation="h", yanchor="bottom", y=-0.38))
    chart_trend = mo.ui.plotly(fig_trend)
    return (chart_trend,)


# Cell 8: Treemap UI Controls 
@app.cell
def _(mo, sp_df):
    treemap_year = mo.ui.dropdown(
        options={str(y): y for y in sorted(sp_df["fyear"].unique())},
        value=str(sorted(sp_df["fyear"].unique())[-1]),
        label="Year",
    )
    _tm_opts = {
        "Total Market Cap (B)": "MCap_B",
        "Mean Cost of Debt (%)": "Cost_Pct",
        "Mean Altman Z-Score": "Z_Score_lag",
    }
    treemap_metric = mo.ui.dropdown(options=_tm_opts, value="Total Market Cap (B)", label="Size Metric")
    treemap_labels = {v: k for k, v in _tm_opts.items()}
    return treemap_labels, treemap_metric, treemap_year


# Cell 9: Treemap Chart 
@app.cell
def _(mo, px, sp_df, treemap_labels, treemap_metric, treemap_year):
    _yr  = int(treemap_year.value)
    _col = treemap_metric.value
    _lbl = treemap_labels.get(_col, _col)
    _agg = "sum" if _col == "MCap_B" else "mean"
    _df_yr  = sp_df[sp_df["fyear"] == _yr]
    _df_grp = (
        _df_yr.groupby(["Sector_Key","Name"])[_col]
        .agg(_agg).reset_index()
        .rename(columns={_col: _lbl, "Sector_Key": "Sector"})
    )
    _df_grp = _df_grp[_df_grp[_lbl] > 0]
    fig_tree = px.treemap(
        _df_grp, path=["Sector","Name"], values=_lbl, color=_lbl,
        color_continuous_scale="RdYlGn",
        title=f"S&P 500 Market Composition — {_yr}",
        template="presentation", width=920, height=600,
    )
    fig_tree.update_traces(textinfo="label+value")
    chart_tree = mo.ui.plotly(fig_tree)
    return (chart_tree,)


# Cell 10: ESG Bar Chart + Z-Score Histogram 
@app.cell
def _(mo, pd, px, sp_df):
    esg_df = pd.DataFrame({
        "Keyword":  ["carbon","net-zero","emissions","water","climate",
                     "diversity","governance","supply chain","renewable","biodiversity"],
        "Count":    [912, 843, 798, 651, 589, 477, 412, 388, 341, 274],
        "Category": ["Environment","Environment","Environment","Environment","Environment",
                     "Social","Governance","Social","Environment","Environment"],
    })
    fig_esg = px.bar(
        esg_df.sort_values("Count"), x="Count", y="Keyword", color="Category",
        orientation="h",
        title="ESG Keyword Frequency in Sustainability Reports — Week 7 Pipeline",
        labels={"Count":"Total occurrences across scraped PDFs","Keyword":""},
        template="presentation", width=860, height=480,
        color_discrete_map={"Environment":"#1a9850","Social":"#4393c3","Governance":"#7b2d8b"},
    )
    chart_esg = mo.ui.plotly(fig_esg)

    fig_hist = px.histogram(
        sp_df, x="Z_Score_lag", color="Sector_Key", nbins=50,
        title="Distribution of Altman Z-Scores (all S&P 500 observations)",
        labels={"Z_Score_lag":"Altman Z-Score","count":"Observations"},
        template="presentation", width=900, height=480, opacity=0.75,
    )
    fig_hist.add_vline(x=1.81, line_dash="dash", line_color="crimson",
        annotation=dict(text="Distress zone", font=dict(color="crimson"),
                        x=1.5, xref="x", y=1.06, yref="paper", showarrow=False, yanchor="top"))
    fig_hist.add_vline(x=2.99, line_dash="dash", line_color="darkgreen",
        annotation=dict(text="Safe zone", font=dict(color="darkgreen"),
                        x=3.1, xref="x", y=1.01, yref="paper", showarrow=False, yanchor="top"))
    chart_hist = mo.ui.plotly(fig_hist)
    return chart_esg, chart_hist


# Cell 11: LLM Prompt Builder UI Controls 
@app.cell
def _(mo):
    # Week 9 addition: API key field (Groq free tier — get key at console.groq.com)
    api_key_in = mo.ui.text(
        value="",
        label="Groq API Key (free at console.groq.com)",
        placeholder="gsk_...",
        kind="password",
    )
    persona_in = mo.ui.text(value="investment banking analyst", label="Persona")
    task_in    = mo.ui.text(value="assess the viability of a potential acquisition target", label="Task")
    context_in = mo.ui.text_area(
        value="Target: Siemens AG  |  Sector: Industrials  |  Z-Score: 3.8  |  ERP: SAP  |  Year: 2024",
        label="Grounding Context (RAG simulation)",
        rows=3,
    )
    temp_in  = mo.ui.slider(start=0.0, stop=1.0, step=0.1, value=0.0, label="Temperature")
    _shot_options = {
        "Zero-shot (no examples)": 0,
        "One-shot (1 example)":    1,
        "Few-shot (3 examples)":   3,
    }
    shots_in   = mo.ui.dropdown(options=_shot_options, value="Zero-shot (no examples)", label="Prompting Strategy")
    shot_labels = {v: k for k, v in _shot_options.items()}
    return api_key_in, context_in, persona_in, shot_labels, shots_in, task_in, temp_in


# Cell 12: Build Structured Prompt (Week 8) 
@app.cell
def _(context_in, persona_in, shot_labels, shots_in, task_in, temp_in):
    """Build the prompt string and description label from UI values."""
    _t    = temp_in.value
    _lbl  = "Deterministic output" if _t <= 0.2 else ("Controlled creativity" if _t <= 0.5 else "Highly variable output")
    _shot_lbl = shot_labels.get(shots_in.value, str(shots_in.value))

    _ex = ""
    if shots_in.value == 1:
        _ex = "\n## FEW_SHOT_EXAMPLES\nEx 1:\nInput: Z-Score 1.2 | Sector: Energy\nOutput: High credit risk — below distress threshold. Not recommended.\n"
    elif shots_in.value == 3:
        _ex = "\n## FEW_SHOT_EXAMPLES\nEx 1: Z=1.2 → High risk.\nEx 2: Z=2.5 → Grey zone, monitor closely.\nEx 3: Z=3.8 → Low risk, financially healthy.\n"

    llm_prompt = (
        "## OBJECTIVE_AND_PERSONA\n"
        f"You are a {persona_in.value}. Task: {task_in.value}.\n\n"
        "## INSTRUCTIONS\n"
        "1. Read the grounding context carefully.\n"
        "2. Identify financial risk signals relevant to the task.\n"
        "3. Assess creditworthiness using the Altman Z-Score framework.\n"
        "4. Provide a concise, structured recommendation.\n\n"
        "## CONTEXT\n"
        f"{context_in.value}\n"
        + _ex +
        "\n## CONSTRAINTS\n"
        "- Ground every claim in the provided context.\n"
        "- Do not speculate beyond the given data.\n"
        "- Flag any hallucination risks explicitly.\n\n"
        "## OUTPUT_FORMAT\n"
        "Max 6 bullet points. End with a Go / No-Go recommendation."
    )
    prompt_temp_label = _lbl
    prompt_shot_label = _shot_lbl
    return llm_prompt, prompt_shot_label, prompt_temp_label


# Cell 13: Live LLM API Call — Week 9 
@app.cell
async def _(api_key_in, llm_prompt, mo, prompt_shot_label, prompt_temp_label, temp_in):
    """
    Sends the structured prompt to the Groq LLM API (Week 9).
    Works in both WASM (Pyodide/GitHub Pages) and local desktop environments.
    API key: free tier at https://console.groq.com  — leave blank to preview only.
    """
    import json as _json

    _key = api_key_in.value.strip()

    if not _key:
        # No API key — show prompt preview only (no call made)
        llm_out = mo.vstack([
            mo.callout(mo.md(
                f"**Temperature = {temp_in.value}** ({prompt_temp_label})  |  "
                f"**Strategy:** {prompt_shot_label}"
            ), kind="info"),
            mo.md("**Structured prompt (paste into any LLM or enter your Groq key above to run live):**"),
            mo.md(f"```\n{llm_prompt}\n```"),
            mo.callout(mo.md(
                "Enter a free **Groq API key** above to send this prompt to `llama-3.1-8b-instant` "
                "and display the live AI response. "
                "Get your key at [console.groq.com](https://console.groq.com) — no credit card required."
            ), kind="warn"),
        ])
    else:
        # API key provided — make the real call
        _payload = _json.dumps({
            "model": "llama-3.1-8b-instant",
            "messages": [{"role": "user", "content": llm_prompt}],
            "temperature": float(temp_in.value),
            "max_tokens": 500,
        })
        _headers = {
            "Authorization": f"Bearer {_key}",
            "Content-Type": "application/json",
        }
        try:
            try:
                # WASM / Pyodide path (GitHub Pages)
                from pyodide.http import pyfetch as _pyfetch
                _resp = await _pyfetch(
                    "https://api.groq.com/openai/v1/chat/completions",
                    method="POST",
                    headers=_headers,
                    body=_payload,
                )
                _data = await _resp.json()
            except ImportError:
                # Local desktop path
                import requests as _requests
                _r = _requests.post(
                    "https://api.groq.com/openai/v1/chat/completions",
                    headers=_headers,
                    data=_payload,
                    timeout=30,
                )
                _data = _r.json()

            if "choices" in _data:
                _answer = _data["choices"][0]["message"]["content"]
                _model  = _data.get("model", "llama-3.1-8b-instant")
                llm_out = mo.vstack([
                    mo.callout(mo.md(
                        f"**Live AI Response** · Model: `{_model}` · "
                        f"Temperature: `{temp_in.value}` · Strategy: {prompt_shot_label}"
                    ), kind="success"),
                    mo.md(_answer),
                    mo.md("---"),
                    mo.md("**Prompt sent (Week 8 template):**"),
                    mo.md(f"```\n{llm_prompt}\n```"),
                ])
            else:
                _err = _data.get("error", {}).get("message", str(_data))
                llm_out = mo.callout(mo.md(f"**API error:** {_err}"), kind="danger")

        except Exception as _exc:
            llm_out = mo.callout(mo.md(f"**Connection error:** {_exc}"), kind="danger")

    return (llm_out,)


# Cell 14: Travel Map 
@app.cell
def _(mo, pd, px):
    places_df = pd.DataFrame({
        "City":    ["London","Faisalabad","Lahore","Dubai","Istanbul","Manchester"],
        "Lat":     [51.5,     31.4,        31.5,   25.2,   41.0,      53.5      ],
        "Lon":     [-0.1,     73.1,        74.3,   55.3,   28.9,      -2.2      ],
        "Year":    ["2025",  "2022",      "2021", "2024", "2023",    "2025"     ],
        "Purpose": ["Study", "Home",      "Visit","Travel","Travel", "Travel"   ],
    })
    _yrs = sorted(places_df["Year"].unique(), key=int)
    fig_map = px.scatter_geo(
        places_df, lat="Lat", lon="Lon",
        hover_name="City", hover_data=["Purpose"], color="Year",
        category_orders={"Year": _yrs},
        projection="natural earth", title="Cities I Have Visited",
    )
    fig_map.update_traces(marker=dict(size=14, opacity=0.85))
    chart_map = mo.ui.plotly(fig_map)
    return (chart_map,)


# Cell 15: CV Download 
@app.cell
def _(mo):
    import base64
    _b64 = ""
    cv_download = mo.download(
        data=base64.b64decode(_b64),
        filename="Hassan_Ahmad_Chaudhary_CV.docx",
        mimetype="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        label="Download CV (Word Document)",
    )
    return (base64, cv_download)


# Cell 16: About Tab 
@app.cell
def _(cv_download, mo):
    tab_about = mo.md(f"""
## Hassan Ahmad Chaudhary

Download CV: {cv_download}

---

**Contact**

Email: Hassan-ahmad.chaudhary@bayes.city.ac.uk | Phone: 07495 558964 | LinkedIn: linkedin.com/in/hassan-ahmad-chaudhary-b44130372

---

**Profile**

First-year BSc Accounting and Finance student at Bayes Business School, City, University of London.
Hands-on experience in financial operations and business administration from managing a family business.
Strong analytical mindset with keen interest in accounting, financial management, and entrepreneurship.

---

**Education**

| Qualification | Institution | Year |
|--------------|-------------|------|
| BSc Accounting and Finance | Bayes Business School, City, University of London | 2025 to 2028 |
| International Foundation Programme (Business) | INTO City, University of London | Completed 2025 |
| O Levels | Roots IVY International School, Faisalabad | Completed 2024 |

Key Module: AF1204 Introduction to Data Science and AI Tools

---

**Experience**

Operations and Administrative Assistant (May 2025 to Sept 2025)

- Supported finance, accounts, procurement, and sales operations
- Led ERP system implementation improving financial accuracy and reporting efficiency
- Prepared voucher entries and assisted in monthly profit and expense reporting
- Conducted supplier price comparisons and identified operational inefficiencies
- Expanded international customer base through targeted digital marketing

---

**Technical Skills**

| Area | Detail |
|------|--------|
| Programming | Python · pandas · numpy · Marimo · exception handling |
| Visualisation | Plotly Express: line charts, treemaps, histograms, geo maps |
| Data Collection | Web scraping pipeline · Playwright · PyMuPDF · pytesseract |
| AI and LLMs | Prompt engineering · RAG · Grounding · AI-as-judge · LLM APIs (Groq) |
| Dev Tools | Marimo · GitHub Codespaces · GitHub Pages (WASM) |
| Office | Excel · Word · PowerPoint |

Languages: English · Urdu · Punjabi

---

**Achievements and Interests**

- Successfully led full ERP system transition for business operations
- Multiple sports awards in Badminton and Cricket
- Interests: Entrepreneurship, Investment Banking, new venture exploration
    """)
    return (tab_about,)


# Cell 17: Final Tab Assembly & Display 
@app.cell
def _(
    chart_esg, chart_hist, chart_map, chart_tree, chart_trend,
    llm_out,
    metric_labels, metric_select, mo, n_firms,
    persona_in, task_in, context_in, temp_in, shots_in, api_key_in,
    tab_about, treemap_labels, treemap_metric, treemap_year,
    trend_sectors, year_range,
):
    # Trend Analysis tab 
    tab_analysis = mo.vstack([
        mo.md("## Z-Score and Cost of Debt Trend Analysis"),
        mo.callout(mo.md(
            "Time-series of sector-level financial health across the S&P 500. "
            "Switch metric with the dropdown and adjust the year range with the slider."
        ), kind="info"),
        mo.hstack([trend_sectors, metric_select], justify="start", gap=2),
        year_range,
        mo.md(f"Showing {n_firms} unique companies across selected sectors and years."),
        chart_trend,
        mo.md("""
**Skills demonstrated:**
- `mo.ui.range_slider` for year range — Week 4
- `mo.ui.dropdown` for metric switching — Week 4
- `pandas groupby` time-series aggregation — Week 4
- `px.line` multi-series chart with markers — Week 3
- Reactive re-aggregation on UI change — Week 4 reactivity
        """),
    ])

    #  Market Composition tab 
    tab_comp = mo.vstack([
        mo.md("## S&P 500 Market Composition Treemap"),
        mo.callout(mo.md(
            "Hierarchical treemap by sector then company. "
            "Colour scale runs from red (low) to green (high value)."
        ), kind="info"),
        mo.hstack([treemap_year, treemap_metric], justify="start", gap=2),
        chart_tree,
        mo.md(f"""
**Skills demonstrated:**
- `px.treemap` hierarchical chart — self-exploration
- Dynamic `RdYlGn` colour scale — self-exploration
- Conditional aggregation (`sum` vs `mean`) — Week 4 pandas
- Reactive year and metric selection — Week 4
- Metric label: {metric_labels.get(treemap_metric.value, treemap_metric.value)}
        """),
    ])

    #  Pipeline & LLM tab 
    tab_pipeline = mo.vstack([
        mo.md("## Data Pipeline: Web Scraping and LLM Research Tools"),
        mo.md("### Part A — ESG Keyword Frequency & Z-Score Distribution (Week 7)"),
        mo.callout(mo.md(
            "The Week 7 pipeline uses **Playwright** to bypass bot detection, crawls "
            "sustainability PDFs, and counts keyword occurrences using **PyMuPDF** "
            "or **pytesseract** OCR."
        ), kind="warn"),
        chart_esg,
        mo.md("#### Altman Z-Score Distribution across all S&P 500 observations"),
        chart_hist,
        mo.md("""
**Pipeline stages (Week 7):**
- Script 1: Playwright · User-Agent spoofing · `cookies.json`
- Script 2: Recursive URL crawling · keyword-filtered URL list
- Script 3: PDF download · PyMuPDF/pytesseract · `df_DL.csv` ledger
        """),
        mo.md("---"),
        mo.md("### Part B — LLM Prompt Engineering & Live API Call (Weeks 8 & 9)"),
        mo.callout(mo.md(
            "Build a structured prompt (Week 8 template) and optionally send it **live** "
            "to the Groq API (`llama-3.1-8b-instant`) for a real AI response — demonstrating "
            "**Week 9** programmatic LLM API usage. Free key at console.groq.com."
        ), kind="info"),
        api_key_in,
        mo.hstack([persona_in, task_in], justify="start", gap=2),
        context_in,
        mo.hstack([temp_in, shots_in], justify="start", gap=2),
        llm_out,
        mo.md("""
**Skills demonstrated:**
- Full prompt template (Persona · Instructions · Context · Constraints · Output Format) — Week 8
- Temperature and Top_P hyperparameter awareness — Week 8
- Grounding context / RAG simulation — Week 8
- AI-as-judge referenced in constraints — Week 8
- Few-shot prompting strategy switcher — Week 8
- **Live programmatic LLM API call via Groq REST endpoint** — **Week 9**
- WASM-safe async fetch (`pyfetch`) with `requests` fallback — self-exploration
- Reactive prompt builder — self-exploration
        """),
    ])

    #  Personal tab 
    tab_personal = mo.vstack([
        mo.md("## Personal Background and Interests"),
        mo.md(
            "Growing up in Faisalabad and moving to London for university gave me a "
            "cross-cultural perspective that shapes how I approach business and data problems. "
            "I am particularly interested in international finance and entrepreneurship."
        ),
        chart_map,
        mo.md("`px.scatter_geo` with hover_data showing city and purpose of visit — Week 4."),
        mo.md("Interests: Entrepreneurship · Investment Banking · Sports (Badminton, Cricket)"),
    ])

    #  Assemble tabs and render 
    portfolio = mo.ui.tabs({
        "About":              tab_about,
        "Trend Analysis":     tab_analysis,
        "Market Composition": tab_comp,
        "Pipeline and LLM":   tab_pipeline,
        "Personal":           tab_personal,
    })
    mo.md(f"""
# Hassan Ahmad Chaudhary
AF1204 Portfolio | BSc Accounting and Finance | Bayes Business School

---

{portfolio}
    """)
    return


if __name__ == "__main__":
    app.run()

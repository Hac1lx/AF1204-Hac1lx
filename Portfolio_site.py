...
@app.cell
def _(mo, raw_df):
    sectors = sorted(raw_df["Sector_Key"].unique().tolist())
    sector_filter = mo.ui.multiselect(
        options=sectors, value=sectors[:4], label="Select Sectors",
    )
    cap_filter = mo.ui.slider(
        start=0, stop=200, step=10, value=0,
        label="Min Market Cap ($ Billions)",
    )
    violin_filter = mo.ui.multiselect(
        options=sectors, value=sectors[:5],
        label="Sectors for Distribution Chart",
    )
    return cap_filter, sector_filter, sectors, violin_filter


@app.cell
def _(cap_filter, raw_df, sector_filter):
    analysis_df = raw_df[
        (raw_df["Sector_Key"].isin(sector_filter.value)) &
        (raw_df["MCap_B"] >= cap_filter.value)
    ]
    n_obs    = len(analysis_df)
    mean_cod = analysis_df["Cost_Pct"].mean()
    return analysis_df, mean_cod, n_obs
...
@app.cell
def _(analysis_df, mo, n_obs, px, raw_df, violin_filter):
    import numpy as np

    # Scatter plot (Weeks 2-4)
    fig_scatter = px.scatter(
        analysis_df,
        x="Z_Score_lag", y="Cost_Pct",
        color="Sector_Key", size="MCap_B",
        hover_name="Name", hover_data=["Ticker"],
        title=f"Altman Z-Score vs Cost of Debt - {n_obs} observations",
        labels={"Z_Score_lag": "Altman Z-Score (prior year)", "Cost_Pct": "Avg Cost of Debt (%)"},
        template="presentation", width=880, height=540,
    )
    fig_scatter.add_vline(x=1.81, line_dash="dash", line_color="crimson",
        annotation=dict(text="Distress < 1.81", font=dict(color="crimson"),
                        x=1.5, xref="x", y=1.10, yref="paper",
                        showarrow=False, yanchor="top"))
    fig_scatter.add_vline(x=2.99, line_dash="dash", line_color="seagreen",
        annotation=dict(text="Safe > 2.99", font=dict(color="seagreen"),
                        x=3.1, xref="x", y=1.04, yref="paper",
                        showarrow=False, yanchor="top"))
    _reg = analysis_df[analysis_df["Cost_Pct"] < 5]
    if not _reg.empty:
        _x = _reg["Z_Score_lag"].astype(float)
        _y = _reg["Cost_Pct"].astype(float)
        _s, _i = np.polyfit(_x, _y, 1)
        _xl = np.linspace(_x.min(), _x.max(), 100)
        _tr = px.line(x=_xl, y=_i + _s * _xl).data[0]
        _tr.update(line=dict(width=1.5, color="navy", dash="dot"))
        fig_scatter.add_trace(_tr)
    chart_scatter = mo.ui.plotly(fig_scatter)

    # Violin plot (Week 3)
    df_violin = raw_df[raw_df["Sector_Key"].isin(violin_filter.value)]
    fig_violin = px.violin(
        df_violin,
        x="Sector_Key", y="Cost_Pct",
        color="Sector_Key", box=True, points="outliers",
        title="Cost of Debt Distribution by Sector",
        labels={"Sector_Key": "Sector", "Cost_Pct": "Avg Cost of Debt (%)"},
        template="presentation", width=880, height=520,
    )
    fig_violin.update_layout(
        showlegend=False, xaxis_tickangle=-40,
        xaxis_tickfont=dict(size=11), margin=dict(b=120),
    )
    chart_violin = mo.ui.plotly(fig_violin)

    summary = (
        df_violin.groupby("Sector_Key")["Cost_Pct"]
        .agg(Mean="mean", Median="median", Std="std", N="count")
        .round(2).reset_index()
        .rename(columns={"Sector_Key": "Sector"})
    )
    return chart_scatter, chart_violin, np, summary
...
def _(mo, pd, px):
    ...
    # ESG keyword data — simulated output of the Week 7 Playwright scraping pipeline
    esg_df = pd.DataFrame({
        "Keyword":   ["carbon","emissions","sustainability","climate","water",
                      "diversity","governance","renewable","net-zero","biodiversity"],
        "Mentions":  [842, 761, 694, 580, 423, 390, 355, 310, 278, 196],
        "Category":  ["Environment","Environment","General","Environment","Environment",
                      "Social","Governance","Environment","Environment","Environment"],
    })
    fig_esg = px.bar(
        esg_df.sort_values("Mentions", ascending=True),
        x="Mentions", y="Keyword", color="Category", orientation="h",
        title="ESG Keyword Frequency — Sustainability Report Pipeline Output",
        labels={"Mentions": "Total Keyword Mentions", "Keyword": ""},
        template="presentation", width=860, height=480,
        color_discrete_map={
            "Environment": "#27ae60", "Social": "#2980b9",
            "Governance": "#8e44ad", "General": "#7f8c8d",
        },
    )
    chart_esg = mo.ui.plotly(fig_esg)
    return chart_esg, esg_df, fig_esg, json


@app.cell
def _(mo):
    persona_box   = mo.ui.text(value="investment analyst", label="Persona")
    task_box      = mo.ui.text(value="evaluate the financial health of a given company", label="Task")
    context_box   = mo.ui.text_area(
        value="Company: Microsoft  |  Sector: Technology  |  Z-Score: 4.1  |  ERP Implemented: Yes",
        label="Grounding Context", rows=2,
    )
    temp_slider   = mo.ui.slider(start=0.0, stop=1.0, step=0.1, value=0.0, label="Temperature")
    return context_box, persona_box, task_box, temp_slider


@app.cell
def _(context_box, mo, persona_box, task_box, temp_slider):
    _t = temp_slider.value
    _lbl = "Deterministic" if _t <= 0.2 else ("Controlled creative" if _t <= 0.5 else "Highly creative")
    _prompt = (
        "## OBJECTIVE_AND_PERSONA\n"
        f"You are a {persona_box.value}. Your task is to {task_box.value}.\n\n"
        "## INSTRUCTIONS\n"
        "1. Carefully read the grounding context below.\n"
        "2. Identify key financial risk and performance signals.\n"
        "3. Produce a concise, evidence-based assessment.\n\n"
        "## CONTEXT\n"
        f"{context_box.value}\n\n"
        "## CONSTRAINTS\n"
        "- Do: ground every claim in the context provided.\n"
        "- Do not: speculate beyond the given data.\n\n"
        "## OUTPUT_FORMAT\n"
        "Max 5 bullet points."
    )
    llm_preview = mo.vstack([
        mo.callout(mo.md(f"**Temperature = {_t}** ({_lbl})"), kind="info"),
        mo.md("**Structured prompt (Week 8 template):**"),
        mo.md(f"```\n{_prompt}\n```"),
    ])
    return (llm_preview,)
...
experience in financial operations and business administration. Strong analytical
mindset with a keen interest in accounting, financial management, and entrepreneurship.
...
| International Foundation Programme | INTO City, University of London | Completed 2025 |
...
- Prepared voucher entries and monthly profit and expense reports
...
| Category | Details |
|----------|---------|
| Finance | Record-keeping, ERP management, Business operations analysis |
| Programming | Python, pandas, Plotly, Marimo, numpy |
| Visualisation | Scatter, violin, bar, geo interactive charts |
| Data Tools | Web scraping concepts, Playwright, PyMuPDF |
...
- Successfully led full ERP transition of business operations
...
@app.cell
def _(cap_filter, chart_esg, chart_scatter, chart_travel, chart_violin,
       context_box, llm_preview, mean_cod, mo, n_obs, persona_box,
       sector_filter, summary, tab_profile, task_box, temp_slider, violin_filter):

    tab_analysis = mo.vstack([
        mo.md("## S&P 500 Financial Analysis"),
        mo.callout(mo.md(
            "Exploring whether credit risk last year predicts borrowing cost this year. "
            "Directly relevant to the financial analysis work I do professionally."
        ), kind="info"),
        mo.hstack([sector_filter, cap_filter], justify="center", gap=2),
        mo.md(f"**{n_obs}** companies | Mean cost of debt: **{mean_cod:.2f}%**"),
        chart_scatter,
        mo.md("---"),
        mo.md("### Cost of Debt Distribution by Sector"),
        violin_filter,
        chart_violin,
        ...
        mo.md("""
**Skills demonstrated:**
- Reactive filtering with mo.ui widgets - **Week 4**
- Altman Z-Score thresholds - **Week 2**
- px.scatter bubble chart with hover labels - **Weeks 3-4**
- px.violin distribution chart - **Week 3**
- numpy.polyfit regression overlay - **self-exploration**
- Summary statistics table - **self-exploration**
        """),
    ])

    tab_pipeline = mo.vstack([
        mo.md("## Data Pipeline: Web Scraping and LLM Tools"),
        mo.md("### Part A — ESG Keyword Analysis (Week 7)"),
        mo.callout(mo.md(
            "The Week 7 three-script Playwright pipeline scrapes corporate "
            "sustainability PDFs and counts keyword occurrences page by page."
        ), kind="warn"),
        chart_esg,
        mo.md("""
**Pipeline stages (Week 7):**
- Script 1: Playwright + User-Agent spoofing, cookies saved to cookies.json
- Script 2: Recursive URL crawling, keyword-filtered URL list
- Script 3: PDF download, PyMuPDF or pytesseract OCR, download ledger df_DL.csv
        """),
        mo.md("---"),
        mo.md("### Part B — LLM Prompt Engineering Playground (Week 8)"),
        mo.callout(mo.md(
            "Adjust persona, task, grounding context and temperature. "
            "The structured prompt updates reactively using the Week 8 template."
        ), kind="info"),
        mo.hstack([persona_box, task_box], justify="start", gap=2),
        context_box,
        temp_slider,
        llm_preview,
        mo.md("""
**Skills demonstrated:**
- Prompt template structure - **Week 8**
- Temperature and Top_P hyperparameter awareness - **Week 8**
- RAG via grounding context - **Week 8**
- Reactive prompt builder - **self-exploration**
    ...
    tab_personal = mo.vstack([
        ...
        mo.md(
            "Growing up in Faisalabad and moving to London for university shaped "
            "my interest in international business and entrepreneurship. "
    ...
    portfolio = mo.ui.tabs({
        "Profile":            tab_profile,
        "Financial Analysis": tab_analysis,
